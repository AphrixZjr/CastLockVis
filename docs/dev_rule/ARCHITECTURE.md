...
> 检测规则、聚类 k、onset/回撤/斜率阈值等参数集中写在 `pipeline/pipeline_json_expert.py` 顶部常量（`T0_ONSET_JUMP` / `T0_ONSET_MIN_N` / `ROLL_HALF` / `SNAPBACK_RETRACE` / `SNAPBACK_SLOPE_MAX` / `N_CLUSTERS` / `N_SEEDS`），便于复现与调参。
