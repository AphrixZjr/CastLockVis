...
CastLock-Vis 用「**宏观群落分布 → 中观序列演化 → 微观个体下钻**」的递进式流水线，结合四个高度联动的视图，帮助分析师回答一个工业问题：**类型「舒适圈」对演员的引力有多大，又有谁、靠什么打破了这层「重力场」。**

**项目网站**：https://aphrixzjr.github.io/CastLockVis/
...
- **A/B → C + 详情（中观→微观）：** 在 A 单击演员、点击 B 熵曲线上的尖峰，或在图例卡片的**演员搜索栏**按姓名选人（等同于在 A 单击），均激活 C（高亮该演员 + 同 `clusterId` 同侪，按 `outcome` 分绿/红）并打开 details-on-demand 面板。选中作品序号仅用于绘制 τ 辅助线，不再作为同侪定义。
...
| `entropy.json` | 1157 × `{actorId,curve:[{n,entropy}]}`（每位演员完整作品序列） | B（白线）、C（交叉参照） |
...
完整报告见 [`docs/data_analysis/分析报告.md`](docs/data_analysis/分析报告.md)（脚本 `01_…05_`、`run_all.py`、图表 `figures/`、`stats.json` 均**只读** `public/data/`、不重算系统统计量）。核心结论：

- **舒适圈（任务 1，视图 A）：** 7 个类型部落的「锁定强度」显著不同——**Adventure/Action 锁得最死**（成功率最低、从未转型比例最高，吻合「硬汉动作」原型），**Crime/Drama 最易「试了又被弹回」**（回弹率最高）。
- **类型锁定（任务 2，视图 B）：** 熵曲线是**先冲高、再在高位焊死**（均值 0.94→2.59，末段斜率≈0），锁定表现为**多样性饱和后的平台期**，而非低多样性——这正是 C 视图把成败判据从熵轴改到类型偏离度 `dist` 的原因。87% 演员在第 6 部前后即出现脱锁尝试（窗口期普遍早开）。
- **重力场（任务 3，视图 D）：** 马尔可夫对角占比在 **early→mid 一次性收紧后趋稳**（0.110→0.126→0.124），并非随生涯线性无限加深——锁定是**早发、早稳**的过程。
- **对齐机制（任务 4，视图 C）：** 三类轨迹分叉干净（成功「绿升」、回弹「冲高回落」、未转型「灰平」）；但 T=0 控制变量解释力有限——**导演异质性方向正确且显著但效应小**（Cliff's δ≈0.11），评分/人气不显著，「叫好不叫座触发回弹」未复现。**真正决定成败的是 T=0 之后能否坐稳（轨迹不回吐），而非起跳时的外部条件**（Gerard Butler 即高导演异质性仍被爆款动作 IP 拉回的反例）。

> 一句话主结论：演员的类型锁定**早发早稳**，约 87% 会在第 6 部前后尝试脱锁，但能否成功主要取决于尝试之后能否坐稳。
...
│   ├── plan/
│   │   ├── TODO.md                    # 有序里程碑 S0–S6（全部已交付的 as-built 日志）
│   │   └── FEATURE_LIST.md            # 模块化功能拆解（Fx.y 编号）
│   ├── dev_rule/
│   │   ├── ARCHITECTURE.md            # 技术栈、数据契约(§5)、store/联动模型(§6)
│   │   └── DESIGN_SYSTEM.md           # 两阶段视觉方案（骨架 → 完备，已定稿）
│   ├── data_analysis/                 # 探索性数据分析：分析报告.md + 01..05 脚本 + figures/ + stats.json
│   └── contribution/config.md         # 环境配置
│   ├── references/                    # Cinemetrics 参考资料与视觉分析
│   ├── mid_doc/                       # 中期归档
│   └── final_doc/                     # 终期材料
...
└── src/                               # 前端源码（四视图、联动 store、通用控件与样式）
    ├── main.tsx · App.tsx
    ├── data/ · store/ · lib/
    ├── components/                    # ViewPanel / DetailsPanel / InteractionGuide
    │   ├── common/                    # ChartLegend · ChartTooltip · ActorSearch（演员搜索）
    │   └── controls/                  # RangeSlider · Toggle · BrushLayer
    ├── views/                         # 四个真实数据视图 + 图例与图表工具
    ├── assets/fonts/                  # 本地内嵌 Alata + Playfair Display（附 OFL）
    └── styles/{tokens,global,fonts}.css  # 设计 token（S6 Cinemetrics 定稿值，换肤主入口）+ 字体
```
...
1. [`docs/plan/TODO.md`](docs/plan/TODO.md) — 有序里程碑 S0–S6，**全部已交付**（as-built 日志）。
